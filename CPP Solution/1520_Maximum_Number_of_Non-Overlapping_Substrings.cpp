        ...
        for (int i = n - 1; i >= 0; i--)
        {
            ...
            if (rangeInd.first == i)
            {
                for (int j = 0; j < 26; j++)
                {
                    ...
                    if (rangeLookup.first != -1 && rangeLookup.first <= i && rangeLookup.second >= rangeInd.first && rangeInd.second >= rangeLookup.second)
                    {
                        rangeInd.first = min(rangeLookup.first, rangeInd.first);
                        rangeInd.second = max(rangeLookup.second, rangeInd.second);
                    }
                    else if (visited[j] != -1 && visited[j] < rangeInd.second)
                    {
                        rangeInd.first = min(rangeLookup.first, rangeInd.first);
                        rangeInd.second = max(rangeLookup.second, rangeInd.second);
                    }
