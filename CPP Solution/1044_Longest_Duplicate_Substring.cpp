...
struct SAIS
{
    ...
    void build(const char /*char*/ s[], int n)
    {
        for (int i = 0; i < n; i++)
            S[i] = s[i];
        suffixArray(n);
    ...
    {
        if (pb[p] == pc - 1 || pb[x] == pc - 1 || pp[pb[p] + 1] - p != pp[pb[x] + 1] - x)
            return 0;
        for (int j = 0; j <= pp[pb[p] + 1] - p; j++)
            if (s[j + p] != s[j + x] || iss[j + p] != iss[j + x])
                return 0;
        return 1;
    }
    ...
    void mkhei(int n)
    {
        ...
        for (int i = 0; i < n; i++)
            if (r[i])
            {
                ...
                while (S[i + hei[r[i]]] == S[SA[r[i] - 1] + hei[r[i]]])
                    hei[r[i]]++;
            }
    }
} sa;
...
int main()
{
    string s = "banana";
    Solution S;
    cout << S.longestDupSubstring(s) << endl;
    return 0;
}
