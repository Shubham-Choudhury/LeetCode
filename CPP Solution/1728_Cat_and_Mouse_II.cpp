...
public:
    ...
    bool dp(int nturn, int cx, int cy, int mx, int my) {
        if (nturn %2 == 1) {
            ...
            if (nturn >= 75) return false;//mouse can't win, it used many moves and it couldnt get the food 
            ...
            if (nturn >= 75) return true;//winner position for the cat, nice block ;)
...
int main() {
    Solution s;
    vector<string>v = {"####F","#C...","M...."};
    cout << s.canMouseWin(v,1,2) << endl;
    return 0;
}